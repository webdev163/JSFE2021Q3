...
export default class TreePage {
  ...
  private static getLocalstorage(): void {
    if (localStorage.getItem('webdev163-tree-settings') !== null) {
      ...
      this.updateUI(
        tree.state.treeNum,
        tree.state.backgroundNum,
        tree.state.lights,
        tree.state.isLightsChecked,
        tree.state.isSnowflakesChecked,
        tree.state.isSoundChecked,
      );
    }
  }
  ...
  private static updateUI(
    ...
    (document.querySelector(`.choose-tree-grid-item[data-num="${treeNum || '1'}"]`) as HTMLInputElement).click();
    (document.querySelector(`.choose-bg-grid-item[data-num="${bgNum || '1'}"]`) as HTMLInputElement).click();
